import type { NextConfigComplete, NextConfigRuntime } from 'next/dist/server/config-shared';
import type { Revalidate } from 'next/dist/server/lib/cache-control';
import 'next/dist/lib/setup-exception-listeners';
import { Worker } from 'next/dist/lib/worker';
import { RSC_SUFFIX, type RSC_SEGMENTS_DIR_SUFFIX, type RSC_SEGMENT_SUFFIX } from 'next/dist/lib/constants';
...
import type { DynamicManifestRoute } from 'next/dist/build/utils';
import type { FallbackRouteParam } from 'next/dist/build/static-paths/types';
import { type NEXT_ROUTER_PREFETCH_HEADER, type RSC_HEADER, type RSC_CONTENT_TYPE_HEADER, type NEXT_DID_POSTPONE_HEADER, type NEXT_ROUTER_SEGMENT_PREFETCH_HEADER, type NEXT_REWRITTEN_PATH_HEADER, type NEXT_REWRITTEN_QUERY_HEADER } from 'next/dist/client/components/app-router-headers';
import { RenderingMode } from 'next/dist/build/rendering-mode';
import { type PrefetchSegmentDataRoute } from 'next/dist/server/lib/router-utils/build-prefetch-segment-data-route';
...
export type StaticWorker = typeof import('next/dist/build/worker') & Worker;
...
export default function build(dir: string, experimentalAnalyze: boolean | undefined, reactProductionProfiling: boolean | undefined, debugOutput: boolean | undefined, debugPrerender: boolean | undefined, noMangling: boolean | undefined, appDirOnly: boolean | undefined, bundler: Bundler | undefined, experimentalBuildMode: 'default' | 'compile' | 'generate' | 'generate-env', traceUploadUrl: string | undefined, debugBuildPaths: {
    app: string[];
    pages: string[];
} | undefined, enabledFeatures?: Record<string, unknown>): Promise<void>;
export {};
